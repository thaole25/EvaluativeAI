...
class ConceptBank:
    def __init__(self, concept_dict, device):
        ...
        self.concept_info.vectors = (
            torch.tensor(np.concatenate(all_vectors, axis=0), requires_grad=False)
            .float()
            .to(device)
        )
...
def get_cavs(X_train, y_train, X_val, y_val, C):
    ...
    train_margin = (
        (np.dot(svm.coef_, X_train.T) + svm.intercept_) / np.linalg.norm(svm.coef_)
    ).T
    margin_info = {
...
def learn_concept_bank(pos_loader, neg_loader, backbone, n_samples, C, device="cuda"):
    ...
    y_train = np.concatenate(
        [np.ones(pos_act[:n_samples].shape[0]), np.zeros(neg_act[:n_samples].shape[0])],
        axis=0,
    )
